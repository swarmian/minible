...
uint32_t millis()
{
    ...
    uint64_t milliseconds = temp_calendar.bit.SECOND*1000;
...
static void ctaphid_write(CTAPHID_WRITE_BUFFER * wb, void * _data, int len)
{
    ...
    if (_data == NULL)
    {
        ...
        if (wb->offset > 0)
        {
            ...
            comms_usb_send_ctap_hid_packet(wb->buf, 0x00, wb->offset);
    ...
    for (i = 0; i < len; i++)
    {
        ...
        {
            //ctaphid_write_block(wb->buf);
            comms_usb_send_ctap_hid_packet(wb->buf, 0x00, sizeof(wb->buf));
            wb->offset = 0;
        }
    }
}
